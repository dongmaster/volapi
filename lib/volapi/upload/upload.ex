defmodule Volexupload do
  ...
  def get_upload_key(name, room) do
    ...
    %{"status_code": status_code, "headers": headers, "body": body} = HTTPoison.get!(upload_key_url, [{"Accept", "application/json"}, {"Referer", "https://volafile.io"}], [{:timeout, :infinity}, {:recv_timeout, :infinity}])
  ...
  def upload({server, room, key}, file_path, filename) do
    ...
    if filename do
      HTTPoison.post!(upload_url, {:multipart, [{:file, file_path, {"form-data", [{"name", "file"}, {"filename", filename}]}, []}]}, [{"Origin", @server}, {"Referer", "https://volafile.io"}], [{:timeout, :infinity}, {:recv_timeout, :infinity}])
    else
      HTTPoison.post!(upload_url, {:multipart, [{:file, file_path, {"form-data", [{"name", "file"}, {"filename", Path.basename(file_path)}]}, []}]}, [{"Origin", @server}, {"Referer", "https://volafile.io"}], [{:timeout, :infinity}, {:recv_timeout, :infinity}])
    end
  end
end
